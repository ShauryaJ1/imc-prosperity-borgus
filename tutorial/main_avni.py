...
PARAMS = {
    ...
    Product.SQUID_INK: {
        ...
        "ema_param": 0.62,  # 0.62 gives 5,513 with clearwidth = 4
        "fft_cutoff": 0.2,
...
class Trader:
    ...
    def ink_fair_value(self, order_depth: OrderDepth, traderObject, timestamp) -> float:
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            # --- Modified section for robust mean estimation using Theil-Sen estimator ---
            ...
            window_len = self.params[Product.SQUID_INK]["history_window"]
            if len(price_history) >= window_len:
                # Extract the most recent window of prices
                window_prices = price_history[-window_len:]
                n = len(window_prices)
                if n >= 2:
                    slopes = []
                    # Compute all pairwise slopes between points
                    for i in range(n - 1):
                        for j in range(i + 1, n):
                            current_slope = (window_prices[j] - window_prices[i]) / (j - i)
                            slopes.append(current_slope)
                    robust_slope = statistics.median(slopes)
                    # Provide the slope of the robust regression line as a separate variable
                    robust_line_slope = robust_slope

                    # Compute intercepts for each data point and take the median
                    intercepts = [window_prices[i] - robust_slope * i for i in range(n)]
                    robust_intercept = statistics.median(intercepts)
                    # Predict the robust mean at the last index (n-1)
                    robust_mean = robust_slope * (n - 1) + robust_intercept
                else:
                    robust_mean = window_prices[0]
                    robust_line_slope = 0
            else:
                robust_mean = np.mean(price_history) if price_history else mmmid_price
                robust_line_slope = 0

            # Compute volatility from the most recent prices (using up to 20 latest prices)
            if len(price_history) >= window_len:
                std_value = np.std(price_history[-1:-21:-1]) if len(price_history) >= 20 else np.std(price_history)
                if std_value == 0:
                    std_value = 1  # Prevent division by zero
                z_score_array.append((mmmid_price - robust_mean) / std_value)
            # --- Modified section ends here ---

            # Update the price history and other trader memory data
    ...
    def make_orders(
        ...
        if product == Product.SQUID_INK and len(price_history) >= self.params[Product.SQUID_INK]["history_window"]:
            ...
            long_ema = (1-beta)/(1-beta**(self.params[Product.SQUID_INK]["history_window"]+1)) * (
                fair_value + sum(beta**(i+1) * price_history[-i-1] for i in range(self.params[Product.SQUID_INK]["history_window"]))
            )
            short_ema = (1-beta)/(1-beta**(self.params[Product.SQUID_INK]["small_window"]+1)) * (
                fair_value + sum(beta**(i+1) * price_history[-i-1] for i in range(self.params[Product.SQUID_INK]["small_window"]))
            )

            if short_ema > long_ema+0.01:
                logger.logs += "going up!\n"
            elif short_ema < long_ema-0.01:
                logger.logs += "going down!\n"
            else:
                logger.logs += "flat...\n"
            logger.logs += f"position: {position}\n"
            logger.logs += f"short_ema = {short_ema}\n long_ema = {long_ema}\n"

            if z_score_array and z_score_array[-1] < (-4 if short_ema < long_ema-0.01 else -2):
                if best_ask_above_fair is not None:
                    bid = round(fair_value)
                    ask = best_ask_above_fair + 100
                logger.logs += f"short_ema > long_ema\nPosition: {position}\nBid:{bid}\nAsk:{ask}"
            elif z_score_array and z_score_array[-1] > (4 if short_ema > long_ema+0.01 else 2):
                if best_bid_below_fair is not None:
                    bid = best_bid_below_fair - 100
            ...
            if position < 0 and fair_value < self.params[Product.SQUID_INK]["entry_price"] * (1-ink_stop_loss_percent) and self.params[Product.SQUID_INK]["entry_price"] != -1:
                if best_ask_above_fair is not None:
                    bid = round(fair_value)
                    ask = best_ask_above_fair + 3
            if position > 0 and fair_value > self.params[Product.SQUID_INK]["entry_price"] * (1+ink_stop_loss_percent) and self.params[Product.SQUID_INK]["entry_price"] != -1:
                if best_bid_below_fair is not None:
                    bid = best_bid_below_fair - 3
                    ask = round(fair_value)
    ...
    def run(self, state: TradingState):
        ...
        if Product.SQUID_INK in self.params and Product.SQUID_INK in state.order_depths:
            ...
            ink_fair_value = self.ink_fair_value(state.order_depths[Product.SQUID_INK], traderObject, state.timestamp)
