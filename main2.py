    ...
    def arb(trader, state: TradingState, symbol, fair_price):
        ...
        for ask_price in state.order_depths[symbol].sell_orders:
            ...
            if ask_price < fair_price:
                buy_amount = min(-ask_amount, trader.POSITION_LIMIT[symbol] - current_position)
                if buy_amount > 0:
                ...
                if state.position[symbol] < 0:
                    buy_amount = min(-ask_amount, -current_position)
                    orders.append(Order(symbol, int(ask_price), int(buy_amount)))
                    current_position += buy_amount
        ...
        for bid_price in state.order_depths[symbol].buy_orders:
            ...
            if bid_price > fair_price:
                sell_amount = min(bid_amount, trader.POSITION_LIMIT[symbol] + current_position)
                if sell_amount > 0:
                ...
                if state.position[symbol] > 0:
                    sell_amount = min(bid_amount, current_position)
                    orders.append(Order(symbol, int(bid_price), -int(sell_amount)))
                    current_position -= sell_amount
...
def load_trader_params(filename):
...
class Trader:
    ...
    def __init__(self):
        ...
        self.mcginley_period = 10

        trader_params = load_trader_params("trader_params.json")
        for key, value in trader_params.items():
            # If key corresponds to a nested dictionary, update accordingly.
    ...
    def resin_risk_spread(self, state: TradingState):
        ...
        orders = []
        orders.append(Order("RAINFOREST_RESIN", math.floor(
            self.default_prices["RAINFOREST_RESIN"] - self.idealProfits["RAINFOREST_RESIN"] / 2 - self.spreads[
    ...
    def kelp_trading_mcginley(self, state: TradingState):
        ...
        if current_position == 0:
            # Not long nor short
            orders.append(
                Order("KELP", math.floor(self.mcginley_prices["KELP"] - self.idealProfits["KELP"] / 2), bid_volume))
            orders.append(
                Order("KELP", math.ceil(self.mcginley_prices["KELP"] + self.idealProfits["KELP"] / 2), ask_volume))

        if current_position > 0:
            # Long position
            orders.append(
                Order("KELP", math.floor(self.mcginley_prices["KELP"] - self.idealProfits["KELP"]), bid_volume))
        ...
        if current_position < 0:
            ...
            orders.append(
                Order("KELP", math.ceil(self.mcginley_prices["KELP"] + self.idealProfits["KELP"]), ask_volume))
    ...
    def kelp_trading_mcginley_risk_spread(self, state: TradingState):
        ...
        if current_position == 0:
            ...
            orders.append(Order("KELP", math.floor(
                self.mcginley_prices["KELP"] - self.idealProfits["KELP"] / 2 - self.spreads["KELP"] / 2), bid_volume))
            orders.append(Order("KELP", math.ceil(
                self.mcginley_prices["KELP"] + self.idealProfits["KELP"] / 2 + self.spreads["KELP"] / 2), ask_volume))

        if current_position > 0:
            # Long position
            orders.append(Order("KELP", math.floor(
                self.mcginley_prices["KELP"] - self.idealProfits["KELP"] - current_position * self.risk - self.spreads[
                    "KELP"] / 2), bid_volume))
            orders.append(Order("KELP", math.ceil(
    ...
    def kelp_trading_ema(self, state: TradingState):
        ...
        if current_position == 0:
            # Not long nor short
            orders.append(
                Order("KELP", math.floor(self.ema_prices["KELP"] - self.idealProfits["KELP"] / 2), bid_volume))
    ...
    def run(self, state: TradingState):
        ...
        logger.print(min(state.order_depths["KELP"].sell_orders) - max(state.order_depths["KELP"].buy_orders) +
                     self.profit_target["KELP"])
        logger.print(min(state.order_depths["RAINFOREST_RESIN"].sell_orders) - max(
            state.order_depths["RAINFOREST_RESIN"].buy_orders) + self.profit_target["RAINFOREST_RESIN"])
